...
const AdminKPIDashboard = () => {
  ...
  const [productsDataVisit, setProductsDataVisit] = useState([]);
...
const kpis = metrics ? [
  { 
    ...
    change: '+23.5%', // luego lo hacemos dinámico 😉
    trend: 'up',
  ...
  { 
    ...
    value: formatNumber(metrics.TotalReturns),
    change: '+1.8%',
    trend: 'up',
 ...
 const loadData = async () => {
    try {
      ...
      console.error("Error al cargar las métricas:", ex);
    } finally {
      setLoading(false);
    }
  };
  ...
  const GetReturnStatus = async () => {
    try{
      ...
      if(!token){
        console.error("No se encontró token de autenticación. Por favor, inicia sesión nuevamente.");
        return;
      }

      const response = await clientAxios.get('/Admin/ReturnStatus', {
        headers: {
      ...
      setReturnStatusData(response.data.data.returnsStatus);
    }catch(ex){
      console.error("Error al obtener las métricas del dashboard:", ex);
    }
  };

  const GetOrderStatust = async () => {
    try{
      const token = localStorage.getItem('ape_token');  
      ...
      const response = await clientAxios.get('/Admin/OrderStatus', {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
        },
      ...
      setOrderStatusData(response.data.data.ordersStatus);
    }catch(ex){
      console.error("Error al obtener las métricas del dashboard:", ex);
    }
  };  

  const GetProductsTopVisit = async () => {
    try{
      const token = localStorage.getItem('ape_token');  
      ...
      const response = await clientAxios.get('/Admin/ProductsTopVisit', {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
        },
      ...
      setProductsDataVisit(response.data.data.productsVisited);
    }catch(ex){
      console.error("Error al obtener las métricas del dashboard:", ex);
    }
  };  

  const GetOrderIncomes = async () => {
    try{
      const token = localStorage.getItem('ape_token');  
      ...
      const response = await clientAxios.get('/Admin/OrderIncomes', {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
        },
  ...
  const GetOverViewMetrics = async () => {
    try{
      ...
      const response = await clientAxios.get('/Admin/MetricsOverView', {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
        },
      ...
      setMetrics(response.data.data.kpis);
    }catch(ex){
      console.error("Error al obtener las métricas del dashboard:", ex);
    }
  };
  ...
  const GetProductsTop = async () => {
    try{
      ...
      const response = await clientAxios.get('/Admin/ProductsTop', {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
        },
      ...
      setProductsData(response.data.data.productsData);
    }catch(ex){
      console.error("Error al obtener las métricas del dashboard:", ex);
    }
  };
  ...
  if (loading) {
    return (
      <Spinner message="Consultando tus métricas..."/>
    );
  }
    ...
    <section className="w-full min-h-screen p-4 lg:p-6">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={loadData} className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl font-medium transition-all shadow-md hover:shadow-lg">
            <RefreshCw size={18} />
            Actualizar Datos
          </button>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600">Por cantidad de visitas</p>
            </div>
          </div>
          <div className="space-y-4">
            {productsDataVisit?.map((product, i) => (
              <div key={i} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                    i === 0 ? 'bg-blue-50 text-blue-600' :
                  ...
                  <div>
                    <p className="font-medium text-gray-900">{product.name}</p>
                    <p className="text-sm text-gray-500">{product.visits} visitas</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-gray-900">{product.visits.toLocaleString()}</p>
                  {/* <p className={`text-sm font-medium ${
                    product.growth.startsWith('+') ? 'text-green-600' : 'text-red-600'
                  }`}>
                    {product.growth}
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={returnStatusData}>
              ...
              <XAxis dataKey="name" stroke="#9ca3af" fontSize={12} />
              <YAxis stroke="#9ca3af" fontSize={12} />
              <Tooltip formatter={(value) => [value, 'Devoluciones']} />
              <Bar dataKey="value" radius={[4, 4, 0, 0]}>
      ...
      {/* <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-r from-emerald-50 to-teal-50 p-5 rounded-2xl border border-emerald-200">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <p className="text-sm text-gray-600">Devoluciones de hoy</p>
        ...
        <div className="bg-gradient-to-r from-purple-50 to-violet-50 p-5 rounded-2xl border border-purple-200">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <p className="text-sm text-gray-600">Mis leads o clientes </p>
